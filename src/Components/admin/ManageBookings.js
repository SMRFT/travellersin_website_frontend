...
const ManageBookings = () => {
    ...
    const calculateBookingFinancials = (booking) => {
        if (!booking) return { subtotal: 0, cgst: 0, sgst: 0, totalTax: 0, totalPayable: 0, discount: 0, netPayable: 0, paid: 0, balanceDue: 0 };
        
        const td = booking.tax_details && typeof booking.tax_details === 'object' ? booking.tax_details : {};
        ...
        if (td.subtotal && parseFloat(td.subtotal) > 0) {
            ...
            const rawAmount = parseFloat(booking.payment_details?.amount || booking.amount || 0);
            subtotal = rawAmount;
            cgst = subtotal * 0.025;
            sgst = subtotal * 0.025;
            totalTax = cgst + sgst;
        ...
        const netPayable = Math.max(0, totalPayable - discount);
        ...
        <>
...

            {
                paymentModal.show && paymentModal.booking && (() => {
                    const fin = calculateBookingFinancials(paymentModal.booking);
                    return (
                        <ModalOverlay onClick={() => setPaymentModal({ show: false, booking: null, amount: '', transactionId: '' })}>
                            <ModalContent onClick={e => e.stopPropagation()}>
                                ...
                                <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.85rem', margin: 0 }}>
                                    Record payment for Booking: <strong style={{ color: '#fff' }}>{paymentModal.booking.booking_id}</strong> ({paymentModal.booking.guest_name})
                                </p>

                                <div style={{ background: 'rgba(255,255,255,0.06)', padding: '0.9rem 1rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', display: 'flex', flexDirection: 'column', gap: '0.35rem', fontSize: '0.85rem' }}>
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', color: 'rgba(255,255,255,0.6)', fontSize: '0.8rem' }}>
                                        <span>CGST (2.5%):</span>
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', color: '#60a5fa', fontWeight: '600', fontSize: '0.82rem' }}>
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: '700', color: '#fff', paddingTop: '0.3rem', borderTop: '1px dashed rgba(255,255,255,0.15)' }}>
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', color: '#f87171', fontWeight: '800', fontSize: '0.92rem', paddingTop: '0.35rem', borderTop: '1px solid rgba(255,255,255,0.15)' }}>
                                ...
                                <div>
                                    ...
                                    <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem' }}>
                                        <ActionBtn 
                                            type="button" 
                                            $color="#3b82f6" 
                                            style={{ fontSize: '0.8rem', padding: '0.4rem', border: '1px solid rgba(59,130,246,0.3)', borderRadius: '6px' }}
                                            onClick={() => {
                                                setPaymentModal({ ...paymentModal, amount: (fin.netPayable * 0.15).toFixed(2) });
                                            }}
                                        >
                                            15% Advance
                                        </ActionBtn>
                                        <ActionBtn 
                                            type="button" 
                                            $color="#10b981" 
                                            style={{ fontSize: '0.8rem', padding: '0.4rem', border: '1px solid rgba(16,185,129,0.3)', borderRadius: '6px' }}
                                            onClick={() => {
                                                setPaymentModal({ ...paymentModal, amount: fin.balanceDue.toFixed(2) });
                                            }}
                                        >
                                            Full Remaining (₹{fin.balanceDue.toFixed(2)})
                                        </ActionBtn>
                                    </div>
                                </div>
                                ...
                                <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                                    <ActionBtn
                                        $color="#10b981"
                                        style={{ background: 'rgba(16,185,129,0.1)', flex: 1, borderRadius: '8px', padding: '0.8rem' }}
                                        onClick={() => {
                                            ...
                                            const newTotalPaid = fin.paid + currentPayment;
                                            const newStatus = newTotalPaid >= fin.netPayable ? 'paid' : 'partially_paid';
                                    ...
                                    <ActionBtn
                                        $color="#ff4d4d"
                                        style={{ background: 'rgba(255,77,77,0.1)', flex: 1, borderRadius: '8px', padding: '0.8rem' }}
